...
def artifacts(home, release, configuration, snapshot_directory, *, uid=None):
    """Plan for explicit local roots; future installation must validate ownership.

    The pinned release supplies bin/nexus-workbench-observer accepting only
    --configuration PATH. It must use C2's Observer lock and foreground loop.
    This generator does not certify or discover that executable.
    """
    ...
    return {
        ...
        "commands": {
            "start": [[launchctl, "enable", service],
                      [launchctl, "bootstrap", domain, str(plist)]],
            "stop": [[launchctl, "disable", service], [launchctl, "bootout", service]],
            "status": [[launchctl, "print", service]],
        },
        # Exact files only. A future versioned package must supply its own
        # reviewed inventory for other release files; never recursively delete.
        "uninstall": {
            "commands": [[launchctl, "disable", service], [launchctl, "bootout", service]],
