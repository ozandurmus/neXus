...
| `static/navigation_ui.js` | `NAV.1` **prototype** navigation model — the left rail *and* the device-detail tab strip; an entry renders only when the shell ships the surface it points at. Its contract, `docs/design/NAVIGATION_INFORMATION_ARCHITECTURE.md`, is **DRAFT — PRODUCT OWNER REVIEW REQUIRED** (companion: `docs/design/LOCAL_CONTROL_PLANE_RUNTIME_AND_ENROLLMENT.md`) |
