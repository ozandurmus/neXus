...
| `static/navigation_ui.js` | `NAV.1` **prototype** navigation model — the left rail *and* the device-detail tab strip; an entry renders only when the shell ships the surface it points at. Its contract, `docs/design/NAVIGATION_INFORMATION_ARCHITECTURE.md`, is **FROZEN — PRODUCT OWNER APPROVED** (companion: `docs/design/LOCAL_CONTROL_PLANE_RUNTIME_AND_ENROLLMENT.md`); `M2`'s accessibility closure (`AC-A11Y-1`..`5`) merged via PR #85, but the feature stays `in_progress` per `project/feature_registry.json` — its `availability_rule` criterion is separately owned by later movements |
