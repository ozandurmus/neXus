...
- `orchestrator_interactive_dashboard_app` **MERGED** — interactive PO +
  Orchestrator workbench (`py scripts/orchestrator.py dashboard`), read+
  relay-write only, own worktree, independent of the checkpoint above (no
  shared-file overlap). Detail: `project/backlog.json` id
  `orchestrator_interactive_dashboard_app`, `relay/NXS-LOCAL-0021`.
