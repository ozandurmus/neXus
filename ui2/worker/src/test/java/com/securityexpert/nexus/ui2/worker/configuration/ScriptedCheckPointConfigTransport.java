...
/**
 * A scriptable Check Point {@link DeviceTransport} test double, keyed by
 * exact command text (never a real socket) -- mirrors {@code
 * worker.inventory.ScriptedCheckPointInventoryTransport}.
 */
final class ScriptedCheckPointConfigTransport implements DeviceTransport {

    private final Map<String, String> outputByCommand;
    private final Map<String, String> hostBySessionId = new HashMap<>();

    ScriptedCheckPointConfigTransport(Map<String, String> outputByCommand) {
