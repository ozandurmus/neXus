...
public final class HttpsVendorExecutor {
     ...
     */
    /** The MC's own name and version from /api/system/version (measured 2026-09-26); the certificate name when absent. */
    private Identity managementCenterIdentity(Target target, Credentials creds, Optional<String> peerName) throws InterruptedException {
        try {
            TextResponse v = client.get(target, HttpsVendorPlan.MC_SYSTEM_VERSION, creds, SHORT, 64 * 1024);
            if (v.ok()) {
                JsonNode j = JSON.readTree(v.body());
                Optional<String> name = Optional.ofNullable(j.path("name").asText(null)).filter(x -> !x.isBlank());
                Optional<String> version = Optional.ofNullable(j.path("version").asText(null)).filter(x -> !x.isBlank())
                        .map(x -> j.hasNonNull("buildNumber") ? x + " (build " + j.path("buildNumber").asText() + ")" : x);
                return new Identity(name.or(() -> peerName), Optional.of("Symantec Management Center"), version);
            }
        } catch (IOException e) {
            LOG.log(System.Logger.Level.INFO, "[MANAGEMENT_CENTER] system/version not read: {0}", e.getClass().getSimpleName());
        }
        return new Identity(peerName, Optional.of("Symantec Management Center"), Optional.empty());
    }
    ...
    private ConfirmOutcome confirmManagementCenter(Target target, Credentials creds) throws IOException, InterruptedException {
        ...
        return new ConfirmOutcome.Confirmed(managementCenterIdentity(target, creds, r.peerName()));
    }
    ...
    private InventoryOutcome inventoryManagementCenter(Target target, Credentials creds) throws IOException, InterruptedException {
        ...
        return new InventoryOutcome.Completed(List.of(), managed, names, managementCenterIdentity(target, creds, r.peerName()));
    }
