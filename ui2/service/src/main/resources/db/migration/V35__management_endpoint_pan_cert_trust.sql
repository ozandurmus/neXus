-- Endpoint-scoped Palo Alto certificate authorization with immutable history.
CREATE TABLE management_endpoint_pan_cert_trust (
    trust_entry_id TEXT PRIMARY KEY,
    vendor TEXT NOT NULL CHECK (vendor = 'PALO_ALTO'),
    ...
    superseded_by TEXT REFERENCES management_endpoint_pan_cert_trust(trust_entry_id)
...
CREATE UNIQUE INDEX management_endpoint_pan_cert_trust_active
    ON management_endpoint_pan_cert_trust(management_address, management_port)
    WHERE status = 'ACTIVE';

CREATE FUNCTION fn_audit_management_endpoint_pan_cert_trust() RETURNS TRIGGER
...
BEGIN
    ...
    IF (TG_OP = 'INSERT' AND (NEW.status <> 'ACTIVE' OR NEW.authorized_by <> actor
        OR action NOT IN ('discovery_pan_trust_enroll', 'discovery_pan_trust_re_enroll')))
        OR (TG_OP = 'UPDATE' AND action <> 'discovery_pan_trust_re_enroll') THEN
    ...
    safe_after := jsonb_build_object('trust_entry_id', NEW.trust_entry_id,
        ...
        'status', NEW.status, 'authorized_at', NEW.authorized_at,
        'observed_at', NEW.observed_at,
        'classification', CASE WHEN action = 'discovery_pan_trust_re_enroll'
    ...
    INSERT INTO audit_log(table_name, row_pk, operation, actor_fingerprint, action_id, before_state, after_state)
        VALUES ('management_endpoint_pan_cert_trust', NEW.trust_entry_id, TG_OP, actor, action,
            CASE WHEN TG_OP = 'UPDATE' THEN jsonb_build_object('status', OLD.status) ELSE NULL END, safe_after);
    RETURN NEW;
END;
$$;
REVOKE ALL ON FUNCTION fn_audit_management_endpoint_pan_cert_trust() FROM PUBLIC;
CREATE TRIGGER trg_audit_management_endpoint_pan_cert_trust
    BEFORE INSERT OR UPDATE OR DELETE ON management_endpoint_pan_cert_trust
    FOR EACH ROW EXECUTE FUNCTION fn_audit_management_endpoint_pan_cert_trust();
GRANT SELECT, INSERT, UPDATE ON management_endpoint_pan_cert_trust TO ui2_app;
REVOKE DELETE ON management_endpoint_pan_cert_trust FROM ui2_app;
