...
| --- | --- | --- | --- | --- |
| 1 | **Action class.** Failover is `CLASS_2_OPERATIONAL_STATE_CHANGE`; that class has no member today and gaining one is what a future gate authorizes; it is not `CLASS 1`. | `utils/action_taxonomy.py::CLASS_2_OPERATIONAL_STATE_CHANGE` (`permitted=False`, `console_submittable=False`, no member); the CP ClusterXL adapter (`checkpoint/clusterxl_capability_adapter.py::CPClusterXLCapabilityAdapter`) is implemented and unit tested but has no production reference or wiring — `tests/test_op2_a_b_execution_foundation.py` source-scans confirm no production entry point constructs it, no `adapter_resolver` references it outside `tests/`, and the taxonomy class it would serve still has no member. | `SATISFIED_IN_UNWIRED_FOUNDATION` | The class's own `why` text is answered by `OP.2.1`'s gate record, not overridden — this row itself keeps the class memberless. |
...
**Reading this matrix.** Seven of nine items are fully implemented and unit
