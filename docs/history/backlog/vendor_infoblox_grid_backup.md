...
NXS-LOCAL-0369: HTTPS backup path and appliance onboarding built (V64, role appliance V65); no real-environment backup recorded yet.

2026-09-25 00:41: first real backup of the production Grid Manager (device GarantiDNS, WAPI 2.13.7) COMPLETED in 2.2 s -- 1,019,183 bytes, gzip magic verified, downloadcomplete HTTP 200, artefact V1, contents listed (8 entries). Two fixes were needed on the day: the WAPI version is read from the page title when the VERSION line is absent (12029be's predecessor 32ce6c5) and the download GET carries Content-Type application/force-download (415 without it, 12029be). Size matches the Backbox measurement (996 KB) for the same grid. PO question: grid members are not enrolled as devices -- the grid backup is the whole grid database; member enumeration would be a new WAPI read (GET /wapi/v<ver>/member) behind its own gate entry, proposed as infoblox_grid_member_discovery.
