...
| **D4** | ~~Backup credential identity: separate service account per vendor, or reuse the collection identity with elevated rights?~~ **RESOLVED 2026-08-31 — signed off by the security lead.** Option A adopted as the target model, Option C as the pilot mechanism (same identity model, secret custody differs): a **distinct per-vendor backup service account**, `SECURITYEXPERT_CP_BACKUP_SSH_USERNAME` + `_PASSWORD_FILE` / `_PASSWORD`, **no fallback** to `SECURITYEXPERT_CP_CONFIG_SSH_*`; the CP collector resolves it in its constructor and fails the whole CP collection closed — zero device contact — when it is absent (`cp_backup_credentials_unavailable`, RB.3b B11). Transport tunables (port, timeouts, strict-host-key) stay shared with `SECURITYEXPERT_CP_CONFIG_SSH_*`; only principal + secret are distinct. Reuse (Option B) rejected: it grants the always-on inventory credential a device-mutating capability — the failure `D2` was raised to prevent. Secret delivered by the DEV.2.2 read-only mounted-material pattern for the pilot, moving into the `DEPLOY.1` vault as a distinct secret on server arrival (custody change only, no code change). Brief: `docs/design/D4_BACKUP_CREDENTIAL_IDENTITY_DECISION.md`. **PAN implication (owed follow-up, not a blocker):** `RB.2` currently reuses the inventory API key and owes a follow-up to authenticate device-state export with a distinct PAN service account before it advances past `IMPLEMENTED` (`project/backlog.json` `on_hardware_real_env_validation` + `native_backup`). | security lead — **signed off** | `RB.3b` — unblocked on this axis; `RB.2` (owed follow-up) |
