...
### 3.2 Capability maturity states — what mechanically changes (amended; supersedes the `F0`–`F5` ladder)

> **Amended 2026-09-09 under `docs/design/UI2_0_C5_AMENDMENTS_BUNDLE.md` §4,
> per `UI2_0_BASELINE_CONTRACT.md` §5 item 4.** The `F0`–`F5` ladder this
> section originally described (`F2` "Python writes projections, Java
> reads", `F3` "Python executes, Java submits", `F4` shadow-run parity) is
> **withdrawn** — `UI2_0_DEVELOPMENT_WORKFLOW.md` §2, row 1 and row 3.
> `RUNTIME-DIRECTION` (baseline §2, D-1) means UI 2.0 executes every job in
> Java from the first job; there is no rest state, shadow state or
> execution-migration axis with Python anywhere on the device-contact path.
> The replacement below is the workflow's own capability-maturity vocabulary,
> restated here so this design document is internally consistent with it.

A capability — not a Line-1 file (workflow §3.3: "a capability is a semantic
unit") — moves through four maturity states, recorded per capability (not
per legacy `F`-state) as the extraction/implementation record workflow §3.3
requires:

| State | Meaning | Reached by | What UI 2.0 does with it |
| --- | --- | --- | --- |
| **`CAP-SPEC`** | capability spec (workflow §3.1's nine fields) and fixture set exist, committed under the UI 2.0 tree | the **Extract** movement (workflow §3.3a), read-only over Line-1 for reference | not yet renderable — spec/fixtures only |
| **`CAP-OFFLINE`** | implemented in Java against the fixtures; no device contact yet | the **Implement** movement (§3.3b) | UI 2.0 can render/exercise it against fixture data; screens may be built ahead of validation |
| **`CAP-VALIDATED`** | the Product Owner has triggered the Java capability itself, end to end, on a real device (or, for a pure function/local-logic capability, proven by contract + offline tests) and the result is reviewed | the **Validate** movement (§3.3c), a PO-run session, human | the capability's UI 2.0 spec becomes the **product-wide authority** for that capability (baseline §1 item 4); Line-1's own file remains provenance only |
| **`CAP-RELEASED`** | integrated into a screen/flow | the **Integrate** movement (§3.3d) | the capability is a usable part of the product, grouped under its `REL-*` release slice |

Feature/release-slice labels (`REL-DISCOVERY`, `REL-INVENTORY`, `REL-BACKUP`,
`REL-CHECKS`, `REL-FAILOVER-READINESS`, baseline §1) never collide with these
maturity states — a slice groups capabilities that share a maturity state,
it is not itself a state (workflow §2 row 1, Astra 4.2).

Rules that make the states safe (replacing the withdrawn ladder's five
rules 1:1, same intent, no Python execution step):

1. **A capability enters extraction only once its Line-1 source exists**;
   its Line-1 validation status (`REAL_ENV_VALIDATED` / `FIXTURE_ONLY` /
   `UNPROVEN`) is *recorded in the spec* (workflow §3.1 field 8), not an
   entry gate — an unvalidated Line-1 feature may still be extracted as an
   "unproven" spec (workflow §2 row 12).
2. **`CAP-SPEC` and `CAP-OFFLINE` are real, useful resting states** — a
   capability can sit at `CAP-OFFLINE` for a long time; screens may be built
   against its fixtures before validation, same parallelism intent the old
   `F2`/`F3` resting states expressed, now without a Python execution leg.
3. **Nothing skips `CAP-VALIDATED`.** A capability never reaches
   `CAP-RELEASED` — is never presented as validated — without a PO-run,
   real-environment result recorded (device-contacting capabilities) or a
   contract/offline-test record (pure-function capabilities), per the kind
   of validation workflow §3.3 requires. Skipping it would be the
   "assumed, not proven" port the evidence laws forbid.
4. **The capability spec is the contract, not the Line-1 code.** A Line-1
   movement may change a Python producer freely; it does not move the Java
   capability's maturity state, which advances only through its own
   Extract/Implement/Validate/Integrate movements.
5. **Device contact never doubles.** There is no shadow run to double it in
   the first place (rule 3 above); §3.4 (R-08) still governs Line-1-versus-
   Java device-contact coordination while Line-1 continues to run schedules
   for the static product during the transition.
6. **A capability at `CAP-RELEASED` is invoked from the CLI too.** The Java
   service exposes it as a typed job; the thin Java CLI (§8.3) provides the
   operator-facing CLI parity `LOCAL_CONTROL_PLANE_RUNTIME_AND_ENROLLMENT.md`
   and `PCP.1` require — the *product* rule "every UI action has a CLI
   equivalent" survives the language change, `main.py` is not the only CLI
   forever.

**`AG-U2` restated** (baseline §5 item 4): the withdrawn `F3` row cited
`AG-U2` as "one orchestration path, test-enforced in both languages" — a
Python worker claiming a Java-submitted job. With no Python worker on the
execution path, `AG-U2` is restated as: **one job-execution path, in Java,
test-enforced** — every collection/backup run enters through the Java
`AdmissionCoordinator`/step executor (§3.2 of the workflow; §8.4 below), no
second orchestration path exists, and no language-crossing round-trip proof
is needed because there is only one language on the path.

### 3.3 Worked capability states for three current features (illustrative, not authorized; amended)

| Feature | Today | Enters extraction at | Notes |
| --- | --- | --- | --- |
| Device Registry (`PCP.1`) + enrollment (`M9`) | filesystem JSON, CLI + loopback console intents | now (`REAL_ENV_VALIDATED`) | `CAP-SPEC`: registry rules (normalization, duplicate detection, lifecycle, lock) extracted with the `PCP.1` test contract as fixtures; `CAP-OFFLINE`: Java implementation against those fixtures, rows through PostgreSQL (the governed storage movement `PCP` §10 already requires); `CAP-VALIDATED`/`CAP-RELEASED`: PO-run enrollment on a real target, then integrated |
| CE.2 compliance primitives | Python registry + opt-in probe, `AUTOMATED_VALIDATED` | once real-device output-shape validation is recorded (currently owed — see `CURRENT_STATE.md`) | §8.5's worked example describes `CAP-OFFLINE` → `CAP-VALIDATED` for it |
| HA readiness (`OP.0a/0c`) | offline derivation over collected telemetry | now (no device contact; contract-validated) | `CAP-SPEC`/`CAP-OFFLINE`: the verdict roll-up ported with the generated-matrix test (`SAFE_TO_FAILOVER` unreachable) as the primary fixture; `CAP-VALIDATED` by contract + offline tests (pure-function capability, workflow §3.3) |
...
> **`APPROVAL-MODEL` amended 2026-09-09** (`docs/design/
> UI2_0_C5_AMENDMENTS_BUNDLE.md` §4, per `UI2_0_BASELINE_CONTRACT.md` §5 item
> 3, decision D-2c): this section originally required four-eyes approval on
> **every run**, framed as "back up now does not exist as a browser action."
> That is replaced everywhere below by the baseline's binding model:
> four-eyes on **profile version** (§6.5, unchanged) and on **schedule
> authorisation** (§6.7, extended); a **routine run inside an already-
> approved scope — including a Run Now of an approved profile against an
> approved target — needs only the `D7` role and a mandatory reason, no
> second human**; **restore and, later, controlled failover need
> per-operation independent approval** (owned by `C7`, referenced here, not
> specified by this document). §6.6's "back up now" row, this section's
> "three consequences", §6.7's condition table and `AG-J10` (§13) are amended
> accordingly; §6.1's operating-model paragraph and §6.5's profile-version
> four-eyes rule are otherwise unchanged.
...
credential and allowlist contracts, run by the scheduler or the worker.
**Amended (`UI-OPERATIONAL-RUN-NOW`, §6.6):** a browser request may create a
Run Now **job request** for an approved profile against an approved target —
it never executes a step or contacts a device itself; the worker claims and
executes the queued request under the identical rules as a scheduled run.
...
a **policy/intent write** with its own `D7`, or — **amended,
`UI-OPERATIONAL-RUN-NOW`** — a class 1 **job request** that creates a queued
row and contacts no device itself; a class 1 **step** (device contact) runs
only inside a scheduled run, a CLI-started run, or a worker's execution of a
Run Now job request — never inside the HTTP request/response cycle, and
never as a direct console submission:
...
| connectivity check of a profile against one device | `{profile_id, version, device_id}` | **class 0 typed job** (connect + first read-only step only) | `E1`–`E7`, admission coordinator | Java worker |
| "Run Now" of an approved profile against an approved target (**amended, `UI-OPERATIONAL-RUN-NOW`**) | typed job request `{profile_id, version, device_id[], reason}` — creates a `REQUESTED` job row only, no device contact from this request | class 1 **job request**, not a console submission (`utils.action_taxonomy.CLASS_1_RECOVERY_WRITE.console_submittable` stays `False`) | `D7` `role:backup_admin` + mandatory reason (no second approver — routine run inside an approved scope, `APPROVAL-MODEL`); the request is refused at admission with `recovery_write_not_console_submittable`-class reasoning only when the profile version or target is **not** already approved; approval is re-checked fresh at claim time (`C2` §6/§7.3), never assumed from request time | request: Java API. Execution: Java worker only — never inside the HTTP request/response cycle |
| a scheduled backup run | a due `BackupSchedule` (scheduler) or the CLI (`--recovery-collect` today; the Java CLI once released) | class 1 under `RB.x`: per-entity ledger, minimum re-execution interval, distinct backup credential, fail-closed allowlist, mandatory reason (`C-D6`) for CLI-started runs | `E7` admission + immediately-before-execution registry and allowlist re-check | worker |
...
1. **"Run Now" from the UI creates a job request, never a direct device
   action, and the UI says so (amended, `UI-OPERATIONAL-RUN-NOW`)**: for an
   approved profile against an approved target, the backup screen offers
   "Run Now", which requires the `D7` `role:backup_admin` and a mandatory
   reason (no second approver — `APPROVAL-MODEL`); the request queues a job
   the worker executes, never a synchronous browser-triggered device
   contact. For a profile version or target that is **not** approved, the
   screen shows the class 1 refusal (`recovery_write_not_console_
   submittable`-class reasoning) with the next scheduled run, and offers
   "run the connectivity check" as the class 0 alternative — the original
   unconditional refusal survives only for the unapproved case.
...
| **Schedule authorisation is four-eyes (amended, `APPROVAL-MODEL`, D-2c)** | the intent that first **enables** a schedule (`enabled: false → true`), or that changes an already-enabled schedule's `assignment_id`/`device_set_id`/`cadence` in a way that widens its device set or shortens its interval, requires a second `role:backup_admin` distinct from the requester (`authorized_by ≠ created_by`, the same author≠approver mechanism as profile-version approval, §6.5) before it takes effect; the row records both the requester and the authoriser. A **disable**, or a **narrowing** edit (shorter device set, longer interval, tightened window) needs only `D7` + reason, no second approver — it can only reduce unattended contact, never expand it |
| **Audit before mutation** | an immutable `schedule_changes` row (`who, when, before, after, reason, authorized_by?`) is durable before the `BackupSchedule` row changes (`CON.0` §7.9 ordering) |
...
A **routine Run Now** of an already-approved profile against an
already-approved target (§6.6) is deliberately **not** listed above: it does
not touch `BackupSchedule` and needs no schedule authorisation — only the
`D7` role and a mandatory reason (`APPROVAL-MODEL`). `C2` §7.2 resolves the
resulting owner/approver/execution-identity split for both a Run Now and a
scheduled run so neither is ever bound to "whoever last edited the row."
...
### 8.5 Worked example — porting `CE.2` (compliance command primitives) (amended to `CAP-*` terms)

> **Amended 2026-09-09** (`docs/design/UI2_0_C5_AMENDMENTS_BUNDLE.md` §4, per
> baseline §5 item 4): this worked example originally walked `CE.2` through
> the withdrawn `F1`–`F5` ladder, including an `F3` "Python worker executes
> from the queue" step and an `F4` "Java in shadow, Python authoritative"
> step. Neither exists any more (§3.2). The same worked example, restated
> against the extraction/implementation movements and `CAP-*` states:
...
validation against each primitive's real device output shape is owed
(`CURRENT_STATE.md`). Extraction (workflow §3.3a) may proceed regardless —
an unvalidated Line-1 feature is extracted as an "unproven" spec (workflow §2
row 12) — but the resulting Java capability cannot pass `CAP-VALIDATED`
until that real-device evidence exists. (The port below is therefore
described, not scheduled.)

**`CAP-SPEC` — extract.** The capability spec (workflow §3.1's nine fields)
and a fixture set are produced by reading `CE.2`'s registry, redaction
pattern and its 33 existing tests — never importing Line-1 code. The target
projection shape is frozen as `tests/fixtures/ui2_conformance/ce2/
primitive_result.v1.json` (`run_id, device_id/endpoint_id, primitive_id,
vendor, success, error_class, executed_at, bytes, lines,
fingerprint_sha256`), generated from the existing 33 Python tests as
fixtures, not as a live Python-writes/Java-reads runtime relationship.

**`CAP-OFFLINE` — implement.** The Java capability is built against those
fixtures: the registry, gate-reviewed primitive definitions, redaction and
the `primitive_results` projection writer, all in Java, exercised by tests
against the fixture set only — no device contact. UI 2.0 may already render
the capability's Compliance tab (freshness, per-primitive outcome,
fingerprint) from fixture-seeded data at this state; `compliance_probe` is
exposed as a class 0 typed job (`operator` role) whose worker is the Java
capability itself, never a Python queue consumer.

**The Java implementation.** The registry's shape in Java:
...
**Proof required to reach `CAP-VALIDATED` (amended — no Python-parity step; the
Java implementation is validated on its own merits against fixtures and,
finally, a real device):**

| Proof | Source | Passes when |
| --- | --- | --- |
| registration rejection (write-marker commands, wrong class, out-of-range gate values) | the 33 `test_ce2_…` cases, harvested at extraction time as a JSON table of `{candidate, expected_error_class}` (workflow §3.1 fixture rules — the table is a fixture, not a live re-consumption by the still-running Python test) | the Java constructors throw the mapped exception for every row |
...
| engine wiring | a check pack referencing `primitive.<id>.<field>`, evaluated bare vs wired vs omitted | verdicts match the capability spec's parser semantics (workflow §3.1 field 4) for the same fixtures (this proof is shared with the `CE.1` engine's own port) |
| projection parity | the `CAP-SPEC`-frozen `primitive_result.v1.json` | the Java executor writes rows that validate against that schema |
| **real-environment (`CAP-VALIDATED`)** | `docs/reference/REAL_ENV_VALIDATION_PROTOCOL.md`: one bounded, read-only probe per vendor, the Product Owner triggering the **Java** capability itself on the pilot target (workflow §3.3c) | SAFE SUMMARY records `{success, error_class, bytes, lines, fingerprint_sha256}` per primitive for the Java run; no Python comparison run exists to match against — the Java result is evidence on its own, judged against the capability spec's parser semantics, not against a prior Python execution |

Only once every row above is recorded in `project/build_history.json` does
the capability reach `CAP-VALIDATED`, and only then may a screen present it
as usable (`CAP-RELEASED`, the **Integrate** movement). `--compliance-probe`
in Line-1 is not "retired" by this process — Line-1 stops growing (baseline
§1 item 5) but its file remains provenance/reference indefinitely (baseline
§1 item 4); removing it from the Python CLI, if ever wanted, is a separate,
later cleanup decision, not a step this port requires. The pattern
generalises: **the Python capability's fixtures and parser expectations
become the fixture table of its Java port** (workflow §2, "narrowed" —
Python *tests* encode Line-1 page-specific behaviour that is not carried
over; only fixtures and parser expectations are), and the real-environment
verdict is earned by the Java implementation on its own, once, at
`CAP-VALIDATED`.
...
| `AG-J10` (amended, `UI-OPERATIONAL-RUN-NOW`) | No HTTP route ever executes a class 1 step directly or synchronously; a Run Now route only ever inserts a `REQUESTED` job row (`D7` `role:backup_admin` + mandatory reason) for an approved profile/target, re-checked at claim time, never assumed valid from request time; a Run Now against an unapproved profile or target is refused with `recovery_write_not_console_submittable`-class reasoning for every role, and the refusal renders per `CON.0` §9 with the next scheduled run named |
