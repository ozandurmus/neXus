...
**Status:** DRAFT — Product Owner acceptance pending. Not implementation authority until the status line reads FROZEN.
...
Common terms. `NOW` = server UTC at request. `ACTIVE` = devices whose `enrollment_state` is in the set the Inventory screen already treats as enrolled; the engineer reuses that predicate, does not define a new one, and the contract records the set name as `UNKNOWN until the engineer cites it`. `GATEWAYS` = `ACTIVE AND role = 'gateway'`. `LATEST_INV(d)` = `MAX(collected_at)` of `device_inventory_run` for device `d`. `LATEST_CFG(d)` = row of `device_configuration_run` with `is_primary AND device_id = d` and max `collected_at`. Cluster = distinct non-null `devices.cluster_member_ref` over `ACTIVE`; a cluster's members are the devices sharing that value.
...
| Backup targets without an archive | devices with `backup_target IS NOT NULL` and no `backup_artefact` row | `COUNT(ACTIVE) WHERE backup_target IS NOT NULL` | `devices`, `backup_artefact` | query failure only | Backup, `artefact=none` |
...
| Inventory | `vendor`, `unit=cluster`, `inventory_age` (`stale`, `lt24h`, `24h_72h`, `gt72h`, `never`), `hotfix_level` |
...
Population: at completion of any `*_configuration_collect` job whose target has a `cluster_member_ref`, the service ports the existing browser comparison function to Python unchanged in semantics, compares the two members' latest primary runs of equal `read_kind`, writes one row, and discards the texts (Raw-evidence law: only section names and counts persist). `comparable=false` when either member lacks such a run. Backfill migration: one pass over all clusters at deploy time using existing runs. Rollback: drop table; the browser path remains. Row identity for the endpoint: latest `computed_at` per `cluster_ref`.
...
**UI (render harness, `tests/test_html_render_harness.py`):** Overview renders every section from the fixture in `tests/fixtures/uitest/`, including one fixture with all-UNKNOWN sections and one with all-zero counts; no green state at zero; every tile and row has an `href` matching §4.2; exception lists cap at five with `Show all (N)`.
...
**Regression:** full suite, render harness, privacy gate, `tests/test_architecture_convergence.py`.
