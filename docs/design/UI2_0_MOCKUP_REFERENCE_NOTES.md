...
Full artboard sources (`*.dc.html`, plus `canvas.json` index) are committed under `docs/design/ui2_mockups/` so a session without access to the claude.ai artifact can still read them. Logo: `docs/design/ui2_mockups/logo-identity-explorations.png` -- six identity explorations (A Junction, B Evidence, C Trace, D Wordmark, E Aperture, F Continuity), all labelled "concept study, not final artwork"; the wordmark `neXus` with the accented capital X is the constant across all six. **Logo decided 2026-09-09: option D / Wordmark**, conditional on the PO's external-use check (`UI2_0_BASELINE_CONTRACT.md` §2 row `LOGO`). No vector source yet; produce one from the study before a logo-bearing screen ships. Also kept: `codex-portfolio-01-material3.png`, an earlier (2026-09-07) Material 3 board from a separate exploration, useful for its token strip (primary #5754A6, surface #F7F5FC, on-surface #242337, tonal surface #E8E3F7, outline #9CA3C7) and the SEE/VERIFY/TRACE/RECOVER/OPERATE maturity-axis footer -- reference only, the canvas artboards (`M3*.dc.html`) remain the primary M3 source.
