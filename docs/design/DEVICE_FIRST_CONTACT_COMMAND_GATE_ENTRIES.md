...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Check Point identity read — software version (form `UNVERIFIED`: `show version all`, fallback `clish -c "show version all"`) | EC-11: the confirm's one identity read; DA-2: the device's facts are read, never typed | CLASS_0_READ | Check Point Gaia, `ssh_exec`, the landing shell (Expert or Clish per `AGENTS.md` Check Point) | 30 s | none; a failure ends the confirm with its outcome | once per confirm, at most once per operator add action | the one session the confirm opens (T-1 of the discovery contracts, `13F` one-session rule) | non-parseable output → identity `UNKNOWN`, row still created under `13F` §2 | none expected; output is version text | version string carried; no raw output persisted |
