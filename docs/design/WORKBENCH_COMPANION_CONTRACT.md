...
| `usage_summary_only` reads the cache; `_public_shape` / `_cost_for` preserve nullable measurements and four cost sources | Reuse pure projection after bounded input validation. Distinguish missing, corrupt and stale caches before helpers normalize missing input. |
...
|---|---|
| Envelope | Schema version, opaque source-set token, observer generation, last completed scan time, coverage/error enums |
...
the prior valid snapshot; no silent eviction of active observations. Removed
sources become unavailable; pruning transitions never deletes upstream history.
...
Each supported local client starts a short-lived stdio MCP process which reads
the configured companion snapshot. It cannot start or supervise the observer.
...
|---|---|---|
| C1: read adapter | Validated in-memory source projection; tests reject path/symlink escapes, duplicates, malformed/oversize input and arbitrary text; monkeypatch forbidden helpers to fail if called; upstream byte hashes unchanged | Remove adapter; upstream untouched |
