...
def row(movement_id, health="healthy", **fields):
    return {"movement_id": movement_id, "objective": "Synthetic " + movement_id,
            "health": health, "provider": "codex", "stage": "coding",
            "process_status": "running" if health in ("healthy", "silent") else "exited", **fields}
...
def workbench(browser):
    ...
    page.add_init_script("""
      ...
      window.fetch = function (path, options) {
        ...
        if (path === '/api/selected-queue') return Promise.resolve(reply({revision:0,pool:[],entries:[],control:'PAUSED',loop:'NOT_ATTACHED',limit:2,occupied:0,can_start:false}));
...
def select(page, movement_id):
    page.get_by_role("button", name="Synthetic " + movement_id, exact=True).click()
    settle(page)
...
def test_navigation_classification_counts_and_exact_id_deduplication(workbench):
    ...
    expect(page.get_by_role("button", name="Active (6)")).to_have_attribute("aria-pressed", "true")
    expect(page.get_by_role("button", name="Needs you (5)")).to_be_visible()
    expect(page.get_by_role("button", name="Archive (4)")).to_be_visible()
    expect(page.locator("#movement-rows tr")).to_have_count(6)
    ...
    page.get_by_role("button", name="Needs you (5)").click()
    select(page, "B")
    page.get_by_role("button", name="Active (6)").click()
    ...
    page.get_by_role("button", name="Needs you (5)").click()
    expect(page.locator("#movement-rows tr")).to_have_count(5)
    page.get_by_role("button", name="Archive (4)").click()
    expect(page.locator("#movement-rows tr")).to_have_count(4)
    expect(page.locator("#movement-rows")).to_contain_text("Closed · delivery unverified")
...
def test_usage_labels_zeros_and_mixed_totals(workbench):
    ...
    expect(page.locator("#movement-rows")).to_contain_text("Unavailable")
...
def test_archive_limit_order_and_relay_only_selection(workbench):
    ...
    assert page.locator(".movement-button").all_text_contents()[0] == "Synthetic terminal-11"
    page.get_by_role("button", name="Show all", exact=True).click()
    expect(page.locator("#movement-rows tr")).to_have_count(13)
    assert page.locator(".movement-button").all_text_contents()[-1] == "Synthetic missing-time"
...
def test_closed_attention_is_history_not_running(workbench):
    page, load = workbench
    load(board(row("partial", "done", relay_status="CLOSED", phase="done", terminal_outcome="PARTIAL", process_status="exited"),
               row("failed", "done", relay_status="CLOSED", phase="failed", terminal_outcome="DONE", verify_passed=False, process_status="exited")))
    expect(page.get_by_role("button", name="Active (0)")).to_be_visible()
    expect(page.get_by_role("button", name="Needs you (2)")).to_be_visible()
    expect(page.get_by_role("button", name="Archive (2)")).to_be_visible()
    expect(page.locator("#movement-rows tr")).to_have_count(0)
    page.get_by_role("button", name="Archive (2)").click()
    expect(page.locator("#movement-rows tr")).to_have_count(2)
    expect(page.locator("#movement-rows")).to_contain_text("Needs review")


def test_live_validation_wrapper_remains_active_after_engineer_close(workbench):
    page, load = workbench
    load(board(row("wrapper", "done", relay_status="CLOSED", phase="done", process_status="exited", execution_active=True)))
    expect(page.get_by_role("button", name="Active (1)")).to_be_visible()
    expect(page.get_by_role("button", name="Archive (0)")).to_be_visible()


def test_info_explains_preparation_and_preserves_work_on_return(workbench):
    page, load = workbench
    load()
    select(page, "A")
    traffic(page)
    page.get_by_label("Message to selected movement").fill("Keep this draft")
    page.get_by_role("tab", name="Info", exact=True).click()
    expect(page.locator("#workbench-view")).to_be_hidden()
    expect(page.locator("#info-view")).to_be_visible()
    expect(page.locator("#info-view")).to_contain_text("waiting for preparation, not reporting a worker failure")
    poll(page)
    expect(page.get_by_role("tab", name="Info", exact=True)).to_have_attribute("aria-selected", "true")
    page.get_by_role("tab", name="Workbench", exact=True).click()
    expect(page.get_by_label("Message to selected movement")).to_have_value("Keep this draft")
    expect(page.locator("#detail-title")).to_have_text("A")
    page.get_by_role("button", name="What does PO preparation mean?").click()
    expect(page.locator("#preparation-title")).to_be_visible()
    page.get_by_role("tab", name="Info", exact=True).press("ArrowLeft")
    expect(page.locator("#workbench-view")).to_be_visible()
    expect(page.get_by_role("tab", name="Workbench", exact=True)).to_be_focused()
