...
- **Post-Parsing Raw Data Prohibition**: Because discovery, capability measurement, and core parsing models are already established for the supported estate, there is NO operational or architectural necessity to expose, view, or converse using raw live production hostnames, serials, management IPs, or internal topology identities.
- **PO and Agent Communication Standard**: The Product Owner and all AI agents MUST converse, write handovers, and document checkpoints using the deterministic masked pseudonyms (e.g. `FW-TANGO-04`, `FW-JULIET-06`, `FW-BRAVO-02`, `CLS-ROMEO-01`) generated by `TopologyNamePseudonymizer`. Unmasked customer/environment hostnames are strictly prohibited in chat contexts, commit messages, PR descriptions, and repository records.
- **Server & Database Execution (AIView Mentality)**:
  - Agents may connect to development servers/hosts to execute authorized tasks (builds, deployments, automated test runs, service troubleshooting per `HOST_REGISTER.md`).
  - However, any database queries, troubleshooting inspections, or state validations MUST adhere strictly to the **AIView mentality**:
    - Never dump unmasked device tables, endpoint lists, or topology graphs into conversation contexts or reports.
    - Query only opaque references (UUIDs), counts, booleans, or pass through the pseudonymized projection layer.
    - Verify relationships and health states locally; report only derived status.
- **UI Design & Previews**: All UI components, preview mockups, and client-side test fixtures must render solely the AIView masked state (`FW-TANGO-04`, synthetic IPs `192.0.2.x`), never unmasked production assets.
...
2026-09-19 limits applied until that reinstall; the reinstall was done on 2026-09-25, so this amendment is in effect.
